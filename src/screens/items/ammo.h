...
extern Entry ammo_entries[];
extern int ammo_max_entries;

class Ammo : public StatsTemplate {
    private:
        LGFX_Sprite ammo_sprites[1];
    public:
        Ammo() : StatsTemplate(ammo_entries, ammo_max_entries) {setInfoMode(1);}
        void load_ammo_sprites();
        void drawDetails(int cur) override {
            ...
            if (idObrazka >= 0 && idObrazka < 1) {
                ...
                int imgW = ammo_sprites[cur].width();
                int imgH = ammo_sprites[cur].height();
                int drawX = 210 + (220 - imgW) / 2;
                int drawY = 50 + (120 - imgH) / 2;

                ammo_sprites[idObrazka].pushSprite(&tft, drawX, drawY);
            }
        }

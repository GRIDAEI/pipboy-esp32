...
extern Entry bron_entries[];
extern int bron_max_entries;

const int liczba = 3;
class Bron : public StatsTemplate {
    private:
        LGFX_Sprite bron_sprites[liczba];
    public:
        Bron() : StatsTemplate(bron_entries, bron_max_entries) {setInfoMode(1);setEquippable(true);}
        void load_bron_sprites();
        void drawDetails(int cur) override {
            int idObrazka = entries[cur].obrazId;
            Serial.println("Proba rysowania");
            if (idObrazka >= 0 && idObrazka < liczba) {
                tft.fillRect(260, 48, 80, 130, COLOR_BG); 
                Serial.print("FUHHH:      ");
                Serial.println(idObrazka);
                // Pobieramy wymiary wobecnego obrazka
                int imgW = bron_sprites[cur].width();
                int imgH = bron_sprites[cur].height();
                int drawX = 210 + (220 - imgW) / 2;
                int drawY = 50 + (120 - imgH) / 2;

                bron_sprites[idObrazka].pushSprite(&tft, drawX, drawY);
            }
        }
        ...
        if (item.dmg != -1) {
            ...
            tft.drawString("OBR:", startX, startY + 6);
